...
async function fetchFullDetailsAndPopulate(item) {
  try {
    ...
    if (detailsData) {
      ...
      // Atualiza links externos com dados do TMDB
      const nome = item.nome;
      let wikiLink = `https://pt.wikipedia.org/wiki/${encodeURIComponent(nome).replace(/%20/g, '_')}`;
      let imdbLink = `https://www.imdb.com/find?q=${encodeURIComponent(nome)}`;
